  ...
  return (
    <div className="max-w-4xl mx-auto px-8 py-16 space-y-12">
      ...
      <div className="space-y-3">
        <p className="font-mono text-xs uppercase tracking-widest text-cf-orange">
          Week 1 of 6 · In development
        </p>
        <h1>Brand-It</h1>
        <p className="text-cf-text-muted max-w-xl" style={{ fontSize: "1.125rem" }}>
          Self-service co-branded marketing assets for Cloudflare channel partners. Partners pick a
          template, fill their brand kit, and export a polished PDF — no designer required.
        </p>
      </div>
      ...
      <section className="space-y-3">
        ...
        <p className="text-sm text-cf-text-muted">
          Everything currently configured and reachable. Greyed-out routes are planned but not yet
          implemented.
        </p>
        ...
        <div className="rounded-card border border-cf-border overflow-hidden">
          ...
          <div className="grid grid-cols-[180px_1fr_120px] gap-4 px-5 py-2.5 bg-cf-bg-300 border-b border-cf-border">
            <span className="font-mono text-xs uppercase tracking-widest text-cf-text-subtle">
              Path
            </span>
            <span className="font-mono text-xs uppercase tracking-widest text-cf-text-subtle">
              Description
          ...
          {[
            ...
            >
              <span className="font-mono text-sm text-cf-orange group-hover:underline truncate">
                {r.path}
              </span>
              <span className="text-sm text-cf-text-muted">{r.desc}</span>
              <span className="inline-flex items-center">
                <span className="font-mono text-xs bg-cf-orange/10 text-cf-orange rounded-pill px-2 py-0.5">
                  live
                </span>
              </span>
            </Link>
          ...
          {[
            ...
            {
              path: "/brand-kit",
              desc: "Brand kit CRUD — logo, colours, boilerplate, contact",
              week: 2,
            },
            {
              path: "/editor/:id",
              desc: "Form-driven editor with live preview + AI chat",
              week: 2,
            },
            {
              path: "/api/ai/tone-rewrite",
              desc: "POST — rewrite a field in a given tone",
              week: 3,
            },
            {
              path: "/api/ai/translate",
              desc: "POST — translate all text fields (EN → DE/FR/ES)",
              week: 3,
            },
            {
              path: "/api/exports/:id",
              desc: "GET — download a completed PDF/PNG export",
              week: 4,
            },
          ].map((r) => (
            <div
            ...
            >
              ...
              <span className="text-sm text-cf-text-muted">{r.desc}</span>
              <span className="inline-flex items-center">
                <span className="font-mono text-xs bg-cf-border text-cf-text-muted rounded-pill px-2 py-0.5">
                  week {r.week}
                </span>
              </span>
            </div>
            ...
            >
              ...
              <div className="flex-shrink-0 flex flex-col items-center gap-1 pt-0.5">
                <span
                  className={[
                    ...
                    w.done ? "bg-cf-orange text-white" : "bg-cf-border text-cf-text-muted",
                  ].join(" ")}
                >
                  W{w.n}
                </span>
                {w.done && <span className="font-mono text-xs text-cf-success">✓</span>}
              </div>
              {/* Content */}
              <div className="flex-1 min-w-0 space-y-1.5">
                ...
                <ul className="space-y-0.5">
                  {w.items.map((item) => (
                    <li key={item} className="text-xs text-cf-text-muted flex items-start gap-1.5">
      ...
      {/* Stack note */}
      <section className="rounded-card border border-cf-border bg-cf-bg-200 p-6 space-y-2">
        <p className="font-medium text-cf-text text-sm">Tech stack</p>
        <p className="text-sm text-cf-text-muted">
          TanStack Start (SSR) · Hono · Tailwind v4 · Biome · D1 · R2 · KV · Workers AI · Agents SDK
          (DOs) · Cloudflare Queues · Browser Rendering · Cloudflare Access
        </p>
